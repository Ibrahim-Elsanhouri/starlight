...
class HrEmployee(models.Model):
    """Inherits the model hr.employee to add extra functionalities and fields"""
    ...
    equipment_count = fields.Integer(compute='_equipment_count',
                                     string='# Equipments',
                                     help="Count of Equipments")

    def _custody_count(self):
        """To count of all custody contracts"""
        for each in self:
            each.custody_count = self.env['hr.custody'].search_count(
                [('employee', '=',
                  each.id)])
    ...
    def _equipment_count(self):
        ...
        for each in self:
            ...
            each.equipment_count = len(equipment_obj.custody_name.ids)
